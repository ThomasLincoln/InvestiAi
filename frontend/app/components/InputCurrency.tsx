...
export default function InputCurrency({
  ...
  return (
    <div ref={containerRef} className="relative">
      <div
        className="flex items-stretch h-10.5 rounded-xl overflow-hidden
      ...
      >
        <div className="flex items-center pl-3 pointer-events-none shrink-0">
          <span className="text-sm font-semibold text-violet-400">{moeda.simbolo}</span>
        </div>
        <input
          type="text"
          inputMode="numeric"
        ...
        >
          <Bandeira title={moeda.nome} className="h-3.5 w-5 rounded-sm object-cover" />
      ...
      {menuAberto && (
        <ul
          className="absolute right-0 z-20 mt-1.5 w-40
